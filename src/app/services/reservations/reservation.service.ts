...
export class ReservationService {
  ...
  constructor(private firestore: AngularFirestore) {
    this.reservationsCollection =
      firestore.collection<Reservation>('reservations');
  }

  // Get all reservations
  async getReservationsByUserId(userId: string): Promise<Reservation[]> {
    const actions = await this.reservationsCollection.ref
      .where('userId', '==', userId)
      .get();
  ...
  getReservations(): Observable<Reservation[]> {
    return this.reservationsCollection.snapshotChanges().pipe(
      map((actions) =>
        actions.map((a) => {
          const data = a.payload.doc.data() as Omit<
            Reservation,
            'reservationId'
          >;
  ...
  createReservation(
    ...
    return this.firestore
      ...
      .doc(reservationId)
      .set(newReservation); // Use set to include the ID
  }
